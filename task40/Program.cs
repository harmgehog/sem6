...
int[] GetNewArray(int size, int minEdge, int maxEdge)
{
...
// генерация массива
double[] GetDoubleArray(int size, int minEdge, int maxEdge)
{
    double[] arr = new double[size];
    for (int i = 0; i < size; i++)
    {
        arr[i] = new Random().NextDouble()*(maxEdge-minEdge) + minEdge;
    }
    Console.Write("Your new array: ");
...
int[] NewArray()
{
    ...
    int[] arr = GetNewArray(size, minEdge, maxEdge);
    return arr;
}
int[] arr = NewArray();
// описание менюшки
...
Help();
// вызовы всех методов через меню
while (true)
{
